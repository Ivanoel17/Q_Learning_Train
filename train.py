...
class QLearningServerBatch:
    ...
    def save_q_table(self, filename="q_table.npy"):
        # q_table adalah defaultdict(tuple[int,...], float)
        # Simpan dict q_table ke file npy dengan pickle
    ...
    def _handle(self, conn: socket.socket, address: tuple[str, int]):
        #Confirmation for connected Matlab
        ...
        while True:
            try:
                ...
                logging.info(f"Received message length: {msg_length} bytes")

                # Receive the actual message content based on the received length
                ...
                # Prepare the response batch data to send back to MATLAB
                response_data = json.dumps(responses).encode('utf-8')
                response_length = len(response_data)

                # Send the message length (4 bytes, little-endian)
                length_header = struct.pack('<I', response_length)  # <I ensures little-endian, unsigned int
                conn.sendall(length_header)

                # Send the message content (the JSON data)
